...
func (e *evaluator) evaluateModules() {
	...
	for _, module := range e.moduleDefinitions {
		metadata := module.Definition.GetMetadata()
		if visited := func(module *ModuleDefinition) bool {
			for _, v := range e.visitedModules {
				if v.name == module.Name && v.path == module.Path && metadata.Reference().String() == v.definitionReference {
		...
		e.visitedModules = append(e.visitedModules, &visitedModule{module.Name, module.Path, metadata.Reference().String()})
		...
		moduleEvaluator := newEvaluator(e.projectRootPath, module.Path, e.workingDir, module.Definition.FullName(), module.Modules[0].GetBlocks(), vars, e.moduleMetadata, e.visitedModules, e.workspace, moduleIgnores, e.stopOnHCLError)
		module.Modules = moduleEvaluator.EvaluateAll()
...
func (e *evaluator) EvaluateAll() terraform.Modules {
	...
	e.moduleDefinitions = e.loadModules(e.stopOnHCLError)
	...
	modules = append(modules, terraform.NewModule(e.projectRootPath, e.modulePath, e.blocks, e.ignores))
