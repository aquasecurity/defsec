...
func (e *evaluator) loadModule(b *terraform.Block, stopOnHCLError bool) (*ModuleDefinition, error) {
	...
	return &ModuleDefinition{
		...
		Path:       modulePath,
		Definition: b,
		Modules:    terraform.Modules{terraform.NewModule(e.projectRootPath, modulePath, blocks, ignores)},
	}, nil
}

func getModuleBlocks(b *terraform.Block, modulePath string, stopOnHCLError bool) (terraform.Blocks, []terraform.Ignore, error) {

	var options []Option
	if stopOnHCLError {
		options = append(options, OptionStopOnHCLError())
	}

	p := New(options...)
	if err := p.ParseDirectory(modulePath); err != nil {
		return nil, nil, err
	}
	moduleFiles := p.(*parser).files
	...
	for _, file := range moduleFiles {
		...
		for _, fileBlock := range fileBlocks {
			blocks = append(blocks, terraform.NewBlock(fileBlock, moduleCtx, b, nil))
		}
