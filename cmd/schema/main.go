...
func main() {
	b := newBuilder()
	if err := b.fromInput(reflect.ValueOf(state.State{})); err != nil {
		panic(err)
	}
	if err := b.writeFile(schemaPath); err != nil {
		panic(err)
	}
	fmt.Println("done")
}
...
func (b *builder) writeFile(path string) error {
	data, err := json.MarshalIndent(b.schema, "", "  ")
	if err != nil {
		return err
	}
...
func (b *builder) readRego(def *Property, name string, parent reflect.Type, typ reflect.Type, raw interface{}, indent int) error {
	...
	fmt.Println(strings.Repeat("  ", indent) + "[rego]" + name)
