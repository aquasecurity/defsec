...
func Test_VPCNetworkACLs(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NotNil(t, testState.AWS.VPC)
			require.Len(t, testState.AWS.VPC.NetworkACLs, 3)

			var acl *vpc.NetworkACL

			for _, a := range testState.AWS.VPC.NetworkACLs {
...
func Test_VPCSecurityGroups(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NotNil(t, testState.AWS.VPC)
			require.Len(t, testState.AWS.VPC.SecurityGroups, 3)

			sg := testState.AWS.VPC.SecurityGroups[0]
			require.NotNil(t, sg)
...
func addSecurityGroup(t *testing.T, ra *aws2.RootAdapter, spec vpcDetails, api *vpcApi.Client, vpc *vpcApi.CreateVpcOutput) {
	...
	// var securityGroupRules []vpcTypes.SecurityGroupRuleUpdate
	// for _, rule := range spec.securityGroup.sgRules {
	// 	securityGroupRules = append(securityGroupRules, vpcTypes.SecurityGroupRuleUpdate{
	// 		SecurityGroupRule: &vpcTypes.SecurityGroupRuleRequest{
	//
	// 			CidrIpv4:   aws.String(rule.cidrBlock),
	...
	// }
	//
	// _, err = api.ModifySecurityGroupRules(ra.Context(), &vpcApi.ModifySecurityGroupRulesInput{
