...
func NewRootAdapter(ctx context.Context, cfg azure.Config, tracker progress.ServiceTracker) *RootAdapter {
	return &RootAdapter{
		ctx:        ctx,
		tracker:    tracker,
		sessionCfg: cfg,
...
func (a *RootAdapter) SessionConfig() azure.Config {
	return a.sessionCfg
}
...
func (r *resolver) ResolveEndpoint(_, _ string, _ ...interface{}) (azure.Endpoint, error) {
	return azure.Endpoint{
		...
		Source:        azure.EndpointSourceCustom,
	}, nil
}

func createResolver(endpoint string) azure.EndpointResolverWithOptions {
	return &resolver{
		endpoint: endpoint,
	}
}
...
func Adapt(ctx context.Context, state *state.State, opt options.Options) error {
	...
	if result.Account == nil {
		return fmt.Errorf("missing account id for azure account")
	}
