...
func TestCheckTableEncryptionCustomerKey(t *testing.T) {
	tests := []struct {
		name     string
		...
		{
			name: "Table missing KMS key name.",
			input: bigquery.BigQuery{
		...
		{
			name: "Table with KMS key name provided.",
			input: bigquery.BigQuery{
				Tables: []bigquery.Table{
					{
						Metadata: defsecTypes.NewTestMetadata(),
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			results := CheckTableEncryptionCustomerKey.Evaluate(&testState)
			var found bool
			for _, result := range results {
				if result.Status() == scan.StatusFailed && result.Rule().LongID() == CheckTableEncryptionCustomerKey.Rule().LongID() {
					found = true
				}
			}
			if test.expected {
